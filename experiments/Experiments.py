    ...
    for d in depth:
        for s in seeds:
            ...
            x_train, x_test, y_train, y_test = train_test_split(
                x, y, test_size=1-train_ratio, random_state=s)
            ...
            print(data, 'cart-d{}'.format(d), 'train acc:', train_acc,
                  'val acc:', val_acc)
    ...
    for d in depth:
        for s in seeds:
            ...
            x_train_enc, x_test_enc, y_train, y_test = train_test_split(
                x_enc, y, test_size=1-train_ratio, random_state=s)
            x_val_enc, x_test_enc, y_val, y_test = train_test_split(
                x_test_enc, y_test, test_size=test_ratio/(test_ratio+val_ratio),
                random_state=s)
            
            for a in alpha:
                # OCT ----------------------------------------------------------
                row = res_oct[
                    (res_oct['instance'] == data) & (res_oct['depth'] == d) & 
                    (res_oct['alpha'] == a) & (res_oct['seed'] == s)]
                if len(row):
                    print(data, 'oct-d{}-a{}'.format(row['depth'].values[0],row['alpha'].values[0]),
                          'train acc:', row['train_acc'].values[0],
                          'val acc:', row['val_acc'].values[0],
                          'gap:', row['gap'].values[0])
                else:
                    octree = miptree.optimalDecisionTreeClassifier(
                        max_depth=d, min_samples_split=0, alpha=a, 
                        warmstart=False, timelimit=timelimit, output=False)
                    tick = time.time()
                ...
                # MFOCT --------------------------------------------------------
                row = res_mfoct[
                    (res_mfoct['instance'] == data) & (res_mfoct['depth'] == d) & 
                ...
                if len(row):
                    ...
                    mfoct = miptree.maxFlowOptimalDecisionTreeClassifier(
                        max_depth=d, alpha=a, warmstart=False,
                    ...
                    train_acc = accuracy_score(
                        y_train, mfoct.predict(x_train_enc))
                    ...
                    res_mfoct = pd.concat(
                        [res_mfoct, pd.DataFrame([row])], ignore_index=True)
            ...
            # BOCT -------------------------------------------------------------
            row = res_boct[
                (res_boct['instance'] == data) &
                (res_boct['depth'] == d) & (res_boct['seed'] == s)]
            if len(row):
                ...
                boct = miptree.binOptimalDecisionTreeClassifier(
                    max_depth=d, min_samples_split=0, warmstart=False,
                    timelimit=timelimit, output=False)
                tick = time.time()
                ...
                row = {'instance':data, 'depth':d, 'seed':s,
                       'train_acc':train_acc, 'val_acc':val_acc,
                       'test_acc':test_acc, 'train_time':train_time,
                       'gap':boct.optgap}
                # res_boct = res_boct.append(row, ignore_index=True)
                res_boct = pd.concat(
                    [res_boct, pd.DataFrame([row])], ignore_index=True)
                res_boct.to_csv('./res/boct.csv', index=False)
                print(data, 'boct-d{}'.format(d), 'train acc:', train_acc,
...
for data in datasets:
    for s in seeds:
        ...
        # data splition
        x_train_enc, x_test_enc, y_train, y_test = train_test_split(
            x_enc, y, test_size=test_ratio, random_state=s)
        N = int(len(y_train) * train_ratio / (train_ratio + val_ratio))
        
        for a in alpha:
            # MFOCT ------------------------------------------------------------
            x_train_enc_s, x_val_enc_s, y_train_s, y_val_s = train_test_split(
                x_train_enc, y_train, 
                test_size=val_ratio/(train_ratio+val_ratio), 
                random_state=3)
            row = res_soct[
                (res_soct['instance'] == data) &
                (res_soct['method'] == 'n') &
                (res_soct['depth'] == d) &
                (res_soct['alpha'] == a) &
                (res_soct['seed'] == s)]
            if len(row):
                ...
                mfoct = miptree.maxFlowOptimalDecisionTreeClassifier(
                    max_depth=2, alpha=a, warmstart=False,
                    timelimit=timelimit, output=False)
                tick = time.time()
                ...
                train_time = tock - tick
                train_acc = accuracy_score(
                    y_train_s, mfoct.predict(x_train_enc_s))
                val_acc = accuracy_score(y_val_s, mfoct.predict(x_val_enc_s))
                test_acc = accuracy_score(y_test, mfoct.predict(x_test_enc))
                row = {'instance':data, 'method':'n', 'depth':d, 'alpha':a,
                       'seed':s, 'train_acc':train_acc, 'val_acc': val_acc,
                       'test_acc':test_acc, 'train_time':train_time,
                       'gap':mfoct.optgap}
                # res_soct = res_soct.append(row, ignore_index=True)
                res_soct = pd.concat(
                    [res_soct, pd.DataFrame([row])], ignore_index=True)
            ...
            if len(row):
                print(data, 'mfoct-d{}-a{}'.format(row['depth'].values[0],row['alpha'].values[0]),
                      'method', row['method'].values[0], 'train acc:', 
                      row['train_acc'].values[0], 'test acc:', 
                      row['test_acc'].values[0], row['gap'].values[0])
            else:
                mfoct = miptree.maxFlowOptimalDecisionTreeClassifier(
                    max_depth=2, alpha=a, warmstart=False,
                    timelimit=timelimit, output=False)
                ...
                test_acc = accuracy_score(y_test, mfoct.predict(x_test_enc))
                row = {'instance':data, 'method':'rb', 'depth':d, 'alpha':a,
                       'seed':s, 'train_acc':train_acc, 'test_acc':test_acc,
            ...
            if len(row):
                ...
                mfoct = miptree.maxFlowOptimalDecisionTreeClassifier(
                    max_depth=2, alpha=a, warmstart=True, 
                    timelimit=timelimit, output=False)
                tick = time.time()
                ...
                train_acc = accuracy_score(y_train, mfoct.predict(x_train_enc))
                test_acc = accuracy_score(y_test, mfoct.predict(x_test_enc))
                row = {'instance':data, 'method':'cp', 'depth':d, 'alpha':a,
                       'seed':s, 'train_acc':train_acc, 'test_acc':test_acc,
                       'train_time':train_time, 'gap':mfoct.optgap}
                # res_soct = res_soct.append(row, ignore_index=True)
                res_soct = pd.concat(
